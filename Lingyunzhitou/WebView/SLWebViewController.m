...
//  Lingyunzhitou
//
//  Created by Marvin Pan on 2020-03-15.
//  Copyright © 2020 Investoday All rights reserved.
//
...
- (void)dealloc{
    ...
    [[_webView configuration].userContentController removeScriptMessageHandlerForName:@"jrtzSignature"];
    [[_webView configuration].userContentController removeScriptMessageHandlerForName:@"jrtzGetSecretId"];
...
- (WKWebView *)webView{
    if(_webView == nil){
        ...
        config.applicationNameForUserAgent = @"Lingyunzhitou";
        ...
        [wkUController addScriptMessageHandler:weakScriptMessageDelegate  name:@"jrtzSignature"];
        [wkUController addScriptMessageHandler:weakScriptMessageDelegate  name:@"jrtzGetSecretId"];
        ...
        NSString *URLString = @"https://sdp-dev.test.investoday.net/#/stockDiagnosis?StockCode=000001&AppId=001&AccountId=10123&ProdId=001&Right=0";
        //JS版本
        // NSString *URLString = @"https://sdp-dev.test.investoday.net/#/stockDiagnosis?StockCode=000001&AppId=001&AccountId=10123&ProdId=001&Right=0&ShowQuotes=1";
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    // JrtzSignature *jrtzSignature = [[JrtzSignature alloc] init];
    ...
    if([message.name isEqualToString:@"jsToOcNoPrams"]){
        ...
        [self presentViewController:alertController animated:YES completion:nil];
    }else if([message.name isEqualToString:@"jrtzGetSecretId"]){
        NSString *promptCode = [NSString stringWithFormat:@"JrtzIOSSignature.getSecretId(\"%@\")",SECRET_ID];
        [_webView evaluateJavaScript:promptCode
           completionHandler:^(id response, NSError * error) {
               if(response != nil) NSLog(@"response: %@", response);
               if(error != nil) NSLog(@"error: %@", error);
           }];

    }else if([message.name isEqualToString:@"jrtzSignature"]){
        NSString *signature = [JrtzSignature jrtzSignatureWithCurrentDateUTC:parameter[@"currentDateUTC"] Product:parameter[@"product"] StringToSign:parameter[@"stringToSign"]];
        
        NSString *promptCode2 = [NSString stringWithFormat:@"JrtzIOSSignature.%@(\"%@\")",parameter[@"method"], signature];
        [_webView evaluateJavaScript:promptCode2
           completionHandler:^(id response, NSError * error) {
               if(response != nil) NSLog(@"response: %@", response);
               if(error != nil) NSLog(@"error: %@", error);
           }];
